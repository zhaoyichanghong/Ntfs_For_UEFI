...
int check_alternate_boot(ntfs_volume *vol)
{
	s64 got_sectors;
	s64 actual_sectors;
	s64 last_sector_off;
  ...
  )
{
	...
	dev = ntfs_device_alloc(name, 0, &ntfs_device_default_io_ops,
			NULL);
	if (!dev) {
		ntfs_log_info(FAILED);
	...
	if ((!ret) && (fix_mftmirr(vol) < 0))
		ret = -1;
	if ((!ret) && (fix_upcase(vol) < 0))
		ret = -1;
	if ((!ret) && (set_dirty_flag(vol) < 0))
		ret = -1;
	if ((!ret) && (empty_journal(vol) < 0))
		ret = -1;
	/*
